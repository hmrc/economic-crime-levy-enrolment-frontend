...
class SessionRepository @Inject() (
  ...
  def upsert(userAnswers: UserAnswers): Future[Boolean] = {
    val updatedAnswers = userAnswers.copy(lastUpdated = Some(Instant.now(clock)))

    collection
      .replaceOne(
        filter = byId(updatedAnswers.internalId),
...
object SessionRepository {
  ...
  val format: Format[UserAnswers] = Json.format[UserAnswers]
}
